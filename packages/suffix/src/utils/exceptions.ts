...
export const mergeExceptionArrays = (
  ...
  rest.reduceRight((acc, curr) => acc.concat(curr.reverse()), []).concat(exceptions.reverse())
...
export const findFirstExceptionOfWord = (
  ...
  mergeExceptionArrays(exceptions, ...rest).find(([wordLike]) => matchesWord(word, wordLike))
