  ...
  isFirstThousand
    ...
    : number
        ...
        .map((digit, index, arr) => {
          ...
          return digit === '2' && twoAsLast
            ? dict.two
            : dict.base[Number(digit)]
        })
        .join('')
  ...
  integer === 0
    ...
    : integer
        ...
        .map((group, index) =>
          group === '000'
            ? ''
            : upToThousand(group, index === 0, integer === 1000) +
              dict.powerOfTen[index],
        )
        .reverse()
        .filter(Boolean)
...
const getPositiveNumberWithWords = (number: number): string => {
  if (number > Number.MAX_SAFE_INTEGER || number < Number.MIN_SAFE_INTEGER) {
    throw new RangeError(
      'You must provide a number that JavaScript can handle.',
    )
  }
