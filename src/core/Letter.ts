...
export class Letter {
  ...
  get isConsonant(): boolean {
    return /^([bcdfghjklmnpqrstvxzwy]|[cz]s|[glnt]y|dzs|[sd]z)$/i.test(
      this.value,
    )
  }
  ...
  get longConsonant(): string {
    if (!this.isConsonant) {
      throw new TypeError(
        'You can call "Letter.longConsonant" only on consonants',
      )
    }
  ...
  private static assertLetter(letter: string): void {
    ...
    if (!hunLetterRegex.test(letter)) {
      throw new TypeError(
        `Cannot use "${letter}" as Letter. Not a valid hungarian letter.`,
      )
    }
  }
}
