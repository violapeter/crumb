...
export class Word {
  ...
  private createLetters(): Array<Letter> {
    const hungarianLettersRegex = /[cz]s|[glnt]y|dzs|[sd]z|[a-záéíóöőúüű]/gi
    ...
    return letters
      ? letters.map((letter, index, array) => {
          if (index < array.length - 1) {
            ...
            if (
              nextLetter.match(/[glnt]y|[cz]s|[sd]z/) &&
              letter === nextLetter.split('')[0]
            ) {
  ...
  get endsWithConsonantCongestion(): boolean {
    ...
    return (
      (this.lastLetter.isConsonant && beforeLastLetter.isConsonant) ||
      this.lastLetter.value === 'dzs'
    )
  }
  ...
  get vowelHeight(): VowelHeight {
    ...
    if (this.vowelHarmony === VowelHarmony.Mixed) {
      ...
      if (
        /[eé]/.test(this.lastVowel.value) ||
        lastTwoVowels.every((vowel) => vowel.isFrontVowel)
      ) {
        return VowelHeight.High
      }
