...
const getSyllablesOfPart = (string: string): string[] => {
  ...
  word.letters.forEach(
    (letter: Letter, index: number, array: Array<Letter>) => {
...
export const getSyllablesOfFullWord = (
  ...
  if (stopWords) {
    ...
    if (stopWordPart) {
      ...
      return getSyllablesOfPart(stopWordPart).concat(
        getSyllablesOfPart(otherPart),
      )
    }
  }
...
export function getSyllables(
  ...
  return options?.formatted
    ? syllables.join(options.hyphenCharacter || '-')
    : syllables
}
